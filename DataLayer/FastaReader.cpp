...
namespace opt {
	...
	/**
	 * Copy BX tag from input SAM line to FASTA/FASTQ comment.
	 * The BX tag is generated by the 'longranger' pipeline
	 * and holds the Chromium droplet barcode for the read.
	 * (If BX tag is absent, FASTA/FASTQ comment will be blank.)
	 */
	int longranger = 0;
...
next_record:
	...
	if (recordType == EOF || m_in.tellg() >= m_end) {
		...
		if (fields.size() >= 11
			...
			if (opt::longranger) {
				// copy Chromium barcode ('BX' tag) to FASTA comment
				for (unsigned i = 11; i < fields.size(); ++i) {
					if (strncmp(fields[i].c_str(), "BX:Z:", 5) == 0)
						comment = fields[i];
				}
			} else {
