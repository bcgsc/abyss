...
TEST(chop_test, trivial_gt_length2) {
  ...
  EXPECT_EQ(9, myString.length()); // sanity check
...
template <typename T> class MultiTypes{
 public:
 	T myVar;
...
TEST(toEng_test, integer_cases) { 
    ...
    EXPECT_EQ("1234", toEng(1234)); // it's just fine without the template!!
    
	MultiTypes<int> snoopy; 
	snoopy.type_int();
	
	snoopy.myVar = 1234;	
	EXPECT_EQ(snoopy.myVar, 1234);
	EXPECT_EQ("1234", toEng(snoopy.myVar));
	
    snoopy.myVar = 12345678;	
	EXPECT_EQ("12.35e6", toEng(snoopy.myVar));

	snoopy.myVar = 123456789;
	EXPECT_EQ ("123.5e6" , toEng(snoopy.myVar));
}

TEST(toEng_test, double_cases){
	MultiTypes<double> snoopy;
	snoopy.type_double();

	snoopy.myVar = 123.456;
	EXPECT_EQ (snoopy.myVar, 123.456); // sanity check
	EXPECT_EQ ("123.5", toEng (snoopy.myVar));
	
	snoopy.myVar = 123456789.9;
	EXPECT_EQ ("123.5e6", toEng(snoopy.myVar));
}

TEST(toEng_test, string_cases){
	MultiTypes<string> snoopy;
	snoopy.type_string();

	snoopy.myVar = "123.456";
	// EXPECT_EQ ("123.5", toEng(snoopy.myVar)); // doesn't work with string! 
}

TEST (startsWith_test, have_no_idea){

	EXPECT_TRUE (startsWith("hello", "hell"));
	EXPECT_TRUE (startsWith("hello", "h"));
	EXPECT_FALSE (startsWith("hello", "hello"));
	EXPECT_TRUE (startsWith("hello", ""));
//	EXPECT_FALSE (startsWith("hello", NULL));
...
TEST(endsWith_test, any_cases){
	...
	//EXPECT_FALSE(endsWith("hello", NULL)); // NULL is not valid
